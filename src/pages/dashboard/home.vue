          ...
          <div class="space-y-3" v-if="account && account.acc_id">
            ...
            <div class="grid grid-cols-1 gap-2 items-start">
              ...
              <div class="bg-base-200 p-2 rounded-md"
                ...
                : 'grey'),}">
                {{ account.department
                ? account.department.dept_shortname : 'Aucun' }}
              </div>
            </div>
            <div class="grid grid-cols-1 gap-2 items-start">
      ...
      <div class="hidden md:block pt-8">
        ...
        <div class="flex justify-center gap-6 py-4">
          ...
          <div class="flex flex-col w-1/2">
            ...
            <div id="left" class="bg-base-200 rounded-lg shadow-md p-4 w-full h-full">
              <!-- Liste des favoris -->
              ...
              <div v-else class="flex items-center justify-center h-full bg-base-100 rounded-lg border-2 border-dashed border-base-300">
                  ...
                  <div class="h-18 w-full flex items-center p-2">
                    ...
                    <div class="w-full flex flex-col">
                      <span class="text-xs opacity-75">
                        {{accord.partnercountry?.parco_name || "Pays indisponible"}}</span>
              ...
              <div :id="'voeu' + i" class="voeuxDrop w-full">
                ...
                >
                  ...
                  >
                    ...
                    <div class="w-full flex flex-col">
                      <span class="text-xs opacity-75">
                        {{localVoeux[i].partnercountry?.parco_name || "Pays indisponible"}}
                      </span>
                      <span class="font-bold text-xs">
                        {{localVoeux[i].university?.univ_city || "Ville indisponible"}}
                        -
                        {{localVoeux[i].university?.univ_name || "Université indisponible"}}
                      </span>
                      <span class="badge badge-xs badge-neutral mt-1">
                        {{localVoeux[i].isced?.isc_code || "Code ISCED indisponible"}}
                      </span>
                    </div>
                  </div>
...
<script setup>
  ...
  async function fetchAll() {
    await request("GET", false, account, config.apiUrl + "api/account/getbylogin/" + accountStore.account.acc_id);
    await request("GET", false, accords, config.apiUrl + "api/agreement/home/");
    await request("GET", false, favoris, config.apiUrl + "api/favoris/me");
    await request("GET", false, admin, config.apiUrl + "api/admin");
    await request("GET", false, destination, config.apiUrl + "api/arbitrage/me");

    await request("GET", false, response, config.apiUrl +
  ...
  async function deleteFile(fileFolder, fileTitle, fileType) {
    closeModal();
    await request("GET", true, response,
