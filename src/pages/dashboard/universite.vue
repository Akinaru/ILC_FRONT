  ...
  <div>
    <div v-if="isLoaded" class="container mx-auto px-4 py-6">
      ...
      <Teleport to="body">
        ...
        <div class="modal modal-bottom sm:modal-middle" role="dialog">
          <div class="modal-box rounded-2xl border border-base-300 shadow-xl" v-if="currentUnivModif">
            <h3 class="text-xl font-bold">Documents de l'université n° {{ currentUnivModif?.univ_id || '' }}</h3>
            <p class="text-sm text-base-content/70 mt-2">Modifiez les documents de l’université sélectionnée.</p>
            <div class="w-full h-px bg-gradient-to-r from-primary/30 via-primary/20 to-transparent my-4"></div>
            ...
            <form @submit.prevent="addDocuments" class="space-y-4" enctype="multipart/form-data">
              ...
              <p class="font-semibold text-md mb-2 text-start w-full pt-3 pl-3">Fichiers de l'université : </p>
              <select v-model="modifiedArticleFiles" class="select select-bordered w-full select-primary h-200px" id="file_select" multiple>
                <div v-for="(document, index) in currentUnivDoc.documents" class="flex justify-between">
...
<script setup>
  ...
  async function modifUnivDocuments(univ){
    modifUniv(univ);
    await request('GET', false, currentUnivDoc, config.apiUrl+'api/documents/university/' + univ.univ_id);
  }
    ...
    if(newfiles.value != null){ //Permet d'enregistrer de nouveaux documents et de les associer à l'univ
      for(const doc of newfiles.value) {
        try{
          ...
          formDataNewDoc.append('folder', '/admin/university/univ_' + currentUnivModif.value.univ_id);
          formDataNewDoc.append('univ_id', currentUnivModif.value.univ_id);

          await request('POST', true, response, config.apiUrl + 'api/documents/university', formDataNewDoc)
          if(response.value.status != 200) return;
        } catch (error){
          console.log("Erreur ajout document: "+error);
        }
      };
      addAction(accountStore.account.acc_id, 'university', response, 'Ajout de documents à l\'université '+ currentUnivModif.value.univ_id +'.');
      await request('GET', false, currentUnivDoc, config.apiUrl+'api/documents/university/' + currentUnivModif.value.univ_id);
      const fileadd = document.querySelector("#file_add");
      fileadd.value = null;
      recapNewDoc.value = [];
      newfiles.value = [];
  ...
  async function deleteDocument(document){
    ...
    await request('DELETE', true, response, config.apiUrl+'api/documents/university', requestData);
    if(response.value.status == 200){
      await request('GET', false, currentUnivDoc, config.apiUrl+'api/documents/university/' + currentUnivModif.value.univ_id);
      addAction(accountStore.account.acc_id, 'university', response, 'Supression d\'un document à l\'université '+ currentUnivModif.value.univ_id +'.');
    }
  }
  onMounted(fetchAll)
</script>