<template>
    ...
    <div class="modal modal-bottom sm:modal-middle" role="dialog">
        <div class="modal-box">
            ...
            <form @submit.prevent="addAgreement" class="w-full *:my-2">
                    <!-- Formulaire Isced -->
                    <label class="form-control w-full items-center justify-center">
                        ...
                        <select class="select select-bordered w-full select-primary" id="isced_select" v-model="newAgreement.isced">
                            <option v-for="(isced, index) in isceds" :key="index" :selected="accord.isc_id === isced.isc_id" :value="isced.isc_id">({{isced.isc_code}}) {{ isced.isc_name }}</option>
